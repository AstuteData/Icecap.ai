...
def hiring_scraping(domain, unique_company_identifier):
        linkedin_url = company_data['linkedin url'][ind]
                ...
                while retries2 < retry_limit2:
                    try:
                        ...
                        for job in job_containers:
                            job_title = job.find('a', class_="disabled ember-view job-card-container__link job-card-list__title").text.strip()
                            job_link_end = job.find('a', class_="disabled ember-view job-card-container__link job-card-list__title")['href'].strip().split('/')
                                ...
                                for job_title, job_link in job_list.items():
                                    ...
                                    while retries3 < retry_limit3:
                                        try:
                                            ...
                                            scraped_job = {"title": job_title, "link": job_link,
                                                           "ul": ul_dict, "p": p_dict}
